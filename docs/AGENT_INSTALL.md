...
> `v3.0.1` 是 FAILED POST-RELEASE VALIDATION 历史，不得安装或解析为 Stable。`v3.0.2` 在完成正式 Release 前只是本地候选，不得从 main 安装；发布成功后再按该 Release 的 Asset digest 与 exact identity 更新本行。
